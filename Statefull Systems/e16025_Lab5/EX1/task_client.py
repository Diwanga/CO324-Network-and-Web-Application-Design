...
#this is testing nondestructive_editTask
def test_edit_nondes(stub) -> None:
	desc = random_string_generator(99)
	task: task_pb2.Task = stub.nondestructive_editTask(task_pb2.Task(id=2, description=desc))
	logging.debug(f"Edited Task {pformat(task)}")
	tasks[task.id] = task.description

# this is testing destructive_editTask
def test_edit(stub) -> None:
	desc = random_string_generator(99)
	task: task_pb2.Task = stub.destructive_editTask(task_pb2.Task(id=2, description=desc))
	logging.debug(f"Edited Task {pformat(task)}")
	tasks[task.id] = task.description
	tasks.pop(2)
...
if __name__ == "__main__":
    ...
    with grpc.insecure_channel("localhost:50051") as channel:
        ...
        # nondestructive_editTask
        test_edit_nondes(stub)
        # listing
        test_list(stub, tasks)
